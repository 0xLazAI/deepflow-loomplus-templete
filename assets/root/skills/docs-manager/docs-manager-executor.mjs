...
const OPENCLAW_ROOT = path.resolve(SKILLS_ROOT, "../..");
const DEFAULT_HOOKS_FILE = path.join(OPENCLAW_ROOT, "hooks", "docs-manager-hooks.json");
const FALLBACK_HOOKS_FILE = path.join(REPO_ROOT, "assets", "root", "hooks", "docs-manager-hooks.json");
...
const USAGE = "usage: node docs-manager-executor.mjs --action <bind|unbind|current|read|write|append|replace|ensure|validate|list|delete|link|locate> --binding-id <bindingId> [--project-code <code>|--path <path-or-alias>|--content <text>|--from <text>|--to <text>|--all|--profile canonical-v1|--force]";
...
async function writeDoc(args) {
  ...
  process.stdout.write(`[docs-manager] writing path=${relPath} bytes=${Buffer.byteLength(content, "utf8")}\n`);
  await fs.writeFile(targetPath, content, "utf8");
  process.stdout.write(`[docs-manager] write done path=${relPath}\n`);
  await maybeNotifyAfterCanonicalWrite({
...
async function appendDoc(args) {
  ...
  await fs.mkdir(path.dirname(targetPath), { recursive: true });
  const content = args.content ?? (await readStdin());
  process.stdout.write(`[docs-manager] appending path=${relPath} bytes=${Buffer.byteLength(content, "utf8")}\n`);
  await fs.appendFile(targetPath, `${content}\n`, "utf8");
  process.stdout.write(`[docs-manager] append done path=${relPath}\n`);
  await maybeNotifyAfterCanonicalWrite({
    bindingId: args.bindingId,
    projectRoot,
    relativePath: relPath,
...
async function replaceDoc(args) {
  ...
  process.stdout.write(`[docs-manager] replacing path=${relPath} from_len=${args.from.length} to_len=${args.to.length} all=${args.replaceAll ? "true" : "false"}\n`);
  await fs.writeFile(targetPath, nextContent, "utf8");
  process.stdout.write(`[docs-manager] replace done path=${relPath}\n`);
  await maybeNotifyAfterCanonicalWrite({
    bindingId: args.bindingId,
    projectRoot,
    relativePath: relPath,
...
async function maybeNotifyAfterCanonicalWrite({ bindingId, projectRoot, relativePath }) {
  const projectId = path.basename(projectRoot);

  const hooks = await loadPostWriteHooks();
  const matchedHooks = hooks.filter((hook) => hook.path === relativePath);
  if (matchedHooks.length === 0) {
    return;
  }

  const targetPath = resolveUnderRoot(projectRoot, relativePath);
  const context = {
    bindingId,
    projectId,
    relativePath,
  ...
  for (const hook of matchedHooks) {
    for (const commandSpec of hook.commands) {
      await runPostWriteHookCommand(commandSpec, context);
    }
  }
}

async function loadPostWriteHooks() {
  const configuredHooksFile = process.env.DOCS_MANAGER_HOOKS_FILE;
  const candidateFiles = configuredHooksFile
    ? [configuredHooksFile]
    : [DEFAULT_HOOKS_FILE, FALLBACK_HOOKS_FILE];

  const resolved = await readFirstExistingHooksFile(candidateFiles);
  if (!resolved) {
    return [];
  }

  const { hooksFile, raw } = resolved;
  ...
  try {
    ...
    process.stderr.write(`[docs-manager] hooks file parse failed file=${hooksFile} reason=${message}\n`);
    return [];
  }
}

async function readFirstExistingHooksFile(candidateFiles) {
  for (const hooksFile of candidateFiles) {
    try {
      const raw = await fs.readFile(hooksFile, "utf8");
      return { hooksFile, raw };
    } catch (error) {
      if (error && error.code === "ENOENT") {
        continue;
      }
      const message = error instanceof Error ? error.message : String(error);
      process.stderr.write(`[docs-manager] hooks file read failed file=${hooksFile} reason=${message}\n`);
      return null;
    }
  }
  return null;
...
function normalizeHookRecord(entry) {
  ...
  const pathValue = typeof entry.path === "string" ? normalizeRelativePath(entry.path) : "";
  if (!pathValue || !Array.isArray(entry.commands) || entry.commands.length === 0) {
    return null;
  }

  const commands = entry.commands
    .map((command) => normalizeHookCommand(command))
...
async function runPostWriteHookCommand(commandSpec, context) {
  const command = applyHookTemplate(commandSpec.command, context);
  const args = commandSpec.args.map((arg) => applyHookTemplate(arg, context));

  try {
    const result = await execCommand(command, args, { cwd: REPO_ROOT });
    ...
    const message = error instanceof Error ? error.message : String(error);
    process.stderr.write(`[docs-manager] hook command failed path=${context.relativePath} command=${command} reason=${message}\n`);
  }
}

function applyHookTemplate(value, context) {
  return String(value)
    .replaceAll("{bindingId}", context.bindingId)
    .replaceAll("{projectId}", context.projectId)
    .replaceAll("{relativePath}", context.relativePath)
    .replaceAll("{projectRoot}", context.projectRoot)
    .replaceAll("{targetPath}", context.targetPath)
    .replaceAll("{openclawRoot}", context.openclawRoot)
