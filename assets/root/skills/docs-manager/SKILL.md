---
name: docs-manager
description: Manage canonical project docs state (bind, read, write, append, replace, ensure, validate, list, link, delete, locate)
---
...
Cross-agent follow-up after canonical writes:

- `write` / `append` / `replace` may trigger asynchronous cross-agent follow-up.
- Docs write success is decoupled from message delivery success.
- `docs-manager` reads post-write hooks from `assets/root/hooks/docs-manager-hooks.json`.
- Matching hooks are executed after content is persisted.
- Follow-up routing should be expressed in the hook commands themselves.
- Queueing and retry behavior can be delegated to the `agent-messenger` skill, but the hook target is configurable.
- The repository default keeps only two demo hook paths: `demo_handoff.md` and `demo_receipt.md`.
...
- Never read canonical docs with direct relative paths outside docs-manager (for example `read 00_meta/decisions.md`), because tools may resolve them under agent workspace paths like `.../workspace-coordinator/...`.
...
- If you need to notify another agent or send owner-visible messages, use `agent-messenger` instead of `docs-manager`.
- The default post-write hook config lives at `assets/root/hooks/docs-manager-hooks.json` and can be overridden with `DOCS_MANAGER_HOOKS_FILE`.
