...
- `assets/`: per-workspace agent assets (`coordinator`, `demo-worker`, `root`).
...
- If you need a quick syntax check for ESM files, use:
  ...
  - `node --check assets/root/skills/docs-manager/docs-manager-executor.mjs`
  - `node --check assets/root/skills/agent-messenger/agent-messenger-executor.mjs`
- Hook mapping config lives at `assets/root/hooks/docs-manager-hooks.json`.
...
- Dockerfile currently installs `clawchef`, `openclaw`, project dependencies, and builds the server.
