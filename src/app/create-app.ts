...
import { createLoomPlusCliApi } from "../loomplus/cli-api.js";
...
type CreateAppOptions = {
  ...
  loomPlusCliTokens: string[];
  loomPlusCliUserId: string;
  loomPlusCliUserEmail: string;
};

export function createApp(options: CreateAppOptions): express.Application {
  const app = express();
  const loomPlusCliApi = createLoomPlusCliApi(options.docsRoot);
  const allowedCliTokens = new Set(options.loomPlusCliTokens.filter((token) => token.length > 0));
  const hasCliAuth = allowedCliTokens.size > 0;
  ...
  app.post("/api/cli/login", async (req, res, next) => {
    try {
      ...
      await loomPlusCliApi.service.ensureStore();
      res.json({
        user: {
          id: options.loomPlusCliUserId,
          email: options.loomPlusCliUserEmail,
        },
      });
    } catch (error) {
      next(error);
  ...
  app.get("/api/cli/tools", async (req, res, next) => {
    try {
      if (!isAuthorizedCliRequest(req, allowedCliTokens, hasCliAuth)) {
        res.status(401).json({ error: { message: "Unauthorized" } });
        return;
      }
      const tools = await loomPlusCliApi.listTools();
      res.json({ tools });
    } catch (error) {
      next(error);
    }
  ...
  app.post("/api/cli/run", async (req, res, next) => {
    try {
      if (!isAuthorizedCliRequest(req, allowedCliTokens, hasCliAuth)) {
        res.status(401).json({ error: { message: "Unauthorized" } });
        return;
      ...
      const args = isPlainObject(req.body?.args) ? (req.body.args as Record<string, unknown>) : {};
      if (!tool) {
        res.status(400).json({ error: { message: "tool is required" } });
        return;
      }
      const result = await loomPlusCliApi.runTool(tool, args);
      res.json({ result });
    } catch (error) {
      next(error);
    }
...
function isAuthorizedCliRequest(req: Request, allowedCliTokens: Set<string>, hasCliAuth: boolean): boolean {
